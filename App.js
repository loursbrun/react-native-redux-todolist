...
function todos(state = [], action) {
  ...
  switch (action.type) {
    case ADD_TODO:
      ...
      newArray.push({ key: newArray.length.toString(), text: action.text, completed: false, selected: true });
      return newArray;
      break;
    case TOGGLE_TODO:
      newArray = state.slice();
      for (let i = 0; i < newArray.length; i++) {
        if (newArray[i].key == action.index) {
          newArray[i] = { key: newArray[i].key, text: newArray[i].text, completed: !newArray[i].completed, selected: true };
        } else {
          newArray[i] = { key: newArray[i].key, text: newArray[i].text, completed: newArray[i].completed, selected: false };
        }
      }
      return newArray;
      break;
...
class AddTodo extends Component {
  ...
  render() {
    return (
      ...
      <View style={{ backgroundColor: '#cacaca', height: 100, marginTop: 50 }}>
        <TextInput
    ...
    return (
      <View style={{ backgroundColor: 'red' }}>
        <TouchableHighlight underlayColor="white" onPress={() => { this.props.onClick(); }}>
          <Text 
          style={ [
            this.props.completed ? [styles.item, styles.completed] : styles.item,
            this.props.selected ? [styles.item, styles.selected] : styles.item 
           ] 
          }
    ...
    return (
      <View style={{ flex: 1, backgroundColor: '#667caa' }}>
        <FlatList
          ...
          renderItem={(o) => <Todo text={o.item.text} completed={o.item.completed} selected={o.item.selected} onClick={() => { this.props.onTodoClick(o.item.key); }} />}
        />
      </View>
    )
  }
